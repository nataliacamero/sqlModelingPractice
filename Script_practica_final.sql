...
CREATE SCHEMA flotakc;

-- Creación de las tablas
CREATE TABLE flotakc.compania_aseguradora (
    id serial primary key,
    nombre varchar(100) not null
);

create table flotakc.poliza(
	id serial primary key,
	matricula varchar(10) not null,
	fecha_alta date not null,
	en_vigor boolean not null,
	id_cpñia_seguradora smallint not null
);

CREATE TABLE flotakc.coche (
...
CREATE TABLE flotakc.grupo_empresa (
    id serial primary key,
    nombre varchar(100) not null
);
...
CREATE TABLE flotakc.color (
    id serial primary key,
    nombre varchar(50) not null
);


-- Restricciones para la tabla flota.poliza
alter table flotakc.poliza add constraint fk_poliza_compania_aseguradora foreign key (id_cpñia_seguradora) references flotakc.compania_aseguradora(id);
alter table flotakc.poliza add constraint fk_poliza_coche foreign key (matricula) references flotakc.coche(matricula);

-- Restricciones para la tabla flota.marca
alter table flotakc.marca add constraint fk_marca_grupo_empresa foreign key (id_grupo_empresa) references flotakc.grupo_empresa(id);

-- Restricciones para la tabla flota.modelo
alter table flotakc.modelo add constraint fk_modelo_marca foreign key (id_marca) references flotakc.marca(id);

-- Restricciones para la tabla flota.coche
alter table flotakc.coche add constraint fk_coche_modelo foreign key (id_modelo) references flotakc.modelo(id);
alter table flotakc.coche add constraint fk_coche_color foreign key (id_color) references flotakc.color(id);

-- Restricciones para la tabla flota.revision
alter table flotakc.revision add constraint fk_revision_coche foreign key (matricula) references flotakc.coche(matricula);
alter table flotakc.revision add constraint fk_revision_moneda foreign key (id_moneda) references flotakc.moneda(id);


-- Definición de la tabla flotakc.automovilesdata
CREATE TABLE flotakc.automovilesdata (
...
-- Inserción de datos
INSERT INTO flotakc.automovilesdata (matricula,grupo,marca,modelo,fecha_compra,color,aseguradora,n_poliza,fecha_alta_seguro,importe_revision,moneda,kms_revision,fecha_revision,kms_totales) VALUES
...
insert into flotakc.grupo_empresa (nombre) select grupo from flotakc.automovilesdata group by grupo;

insert into flotakc.marca (id_grupo_empresa, nombre) select grupo.id, autom.marca from flotakc.automovilesdata autom 
inner join flotakc.grupo_empresa grupo on grupo.nombre = autom.grupo 
group by grupo.id, autom.marca;
	
insert into flotakc.modelo (id_marca, nombre) select mdl.id, autom.modelo from flotakc.automovilesdata autom 
inner join flotakc.grupo_empresa grupo on grupo.nombre = autom.grupo inner join flotakc.marca mdl on mdl.nombre = autom.marca
group by grupo.id, mdl.id, autom.modelo;

insert into flotakc.compania_aseguradora (nombre) select aseguradora from flotakc.automovilesdata group by aseguradora;

insert into flotakc.color (nombre) select color from flotakc.automovilesdata group by color;

insert into flotakc.moneda (nombre) select moneda from flotakc.automovilesdata group by moneda;

insert into flotakc.coche (matricula, fecha_compra, id_modelo, id_color, km) 
select matricula, fecha_compra, mdl.id, c.id, kms_totales from flotakc.automovilesdata autom 
inner join flotakc.grupo_empresa grupo on grupo.nombre = autom.grupo 
inner join flotakc.marca m on m.nombre = autom.marca inner join flotakc.modelo mdl on mdl.nombre = autom.modelo inner join flotakc.color c on c.nombre = autom.color
group by matricula, fecha_compra, mdl.id, c.id, kms_totales;

insert into flotakc.poliza (matricula, id, id_cpñia_seguradora, fecha_alta, en_vigor) select autom.matricula, autom.n_poliza, compaseg.id, autom.fecha_alta_seguro, false from flotakc.automovilesdata autom 
inner join flotakc.compania_aseguradora compaseg on compaseg.nombre = autom.aseguradora 
group by autom.matricula, autom.n_poliza, compaseg.id, autom.fecha_alta_seguro order by autom.matricula, autom.n_poliza ;

update flotakc.poliza p set en_vigor = true from (select p.matricula, max(p.fecha_alta) max_fecha_alta from flotakc.poliza p group by p.matricula) pv 
where p.matricula = pv.matricula and p.fecha_alta = pv.max_fecha_alta;

insert into flotakc.revision (matricula, km, id_moneda, fecha_revision, importe)
select autom.matricula, autom.kms_revision, m.id, autom.fecha_revision, round(autom.importe_revision::decimal, 2) from flotakc.automovilesdata autom
inner join flotakc.moneda m on m.nombre = autom.moneda
group by autom.matricula, autom.kms_revision, m.id, autom.fecha_revision, autom.importe_revision
order by autom.matricula;

-- Se borra la tabla;
drop table flotakc.automovilesdata;

-- Consulta solicitada en el enunciado;
select  
    ge.nombre as grupo, 
    md.nombre as modelo, 
    mr.nombre as marca, 
    ...
    cch.matricula, 
    c.nombre as color, 
    cch.km as total_kilometros, 
    a.nombre as aseguradora, 
    p.id as poliza, 
    rev.fecha_revision, 
    rev.km as km_revision, 
    rev.importe as importe_revision, 
    mon.nombre as moneda
from
    flotakc.coche cch
inner join
    flotakc.modelo md on md.id = cch.id_modelo
inner join
    flotakc.marca mr on mr.id = md.id_marca
inner join
    flotakc.grupo_empresa ge on ge.id = mr.id_grupo_empresa
inner join
    flotakc.poliza p on p.matricula = cch.matricula and p.en_vigor = true
inner join 
    flotakc.compania_aseguradora a on a.id = p.id_cpñia_seguradora
inner join 
    flotakc.color c on c.id = cch.id_color
inner join
    flotakc.revision rev on rev.matricula = cch.matricula
inner join
    flotakc.moneda mon on mon.id = rev.id_moneda;
